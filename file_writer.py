...
class FileWriter:
    @staticmethod
    def write_to_file(
        file_name: str, dict_name: str, db: dict, to_master: bool, sorted: bool = False
    ) -> None:
        if not to_master:
    ...
    def sort_and_write(users_dict: dict, words_dict: dict, to_master: bool) -> None:
        ...
        FileWriter.write_to_file(
            "sorted_users", "USERS", sorted_users, to_master, sorted=True
        )
        FileWriter.write_to_file("words", "WORDS", words, to_master)
        FileWriter.write_to_file(
            "sorted_words", "WORDS", sorted_words, to_master, sorted=True
        )
